sap.ui.define([
	...
	"use strict";
	var i;
	return e.extend("stipAdmin.stipAdmin.controller.KPIsenzacurva", {
		onInit: function () {
		/**** ONINIT FUNCTION IS CALLED UPON INITIALIZATION OF THE VIEW AND CONTROLLER PERFORM THE SETUP ****/
			...
			this.oRouter.getRoute("KPIsenzacurva").attachPatternMatched(this._onObjectMatched, this);
		},
		
		onFlagChange: function () {
		/***** THIS FUNCTION IS CALLED BY THE COMBOBOX IN VIEW AND THE VALUE STATE IS GIVEN NONE *****/
			this.byId("sel26a").setValueState(sap.ui.core.ValueState.None);
			this.byId("sel26b").setValueState(sap.ui.core.ValueState.None);
		},
		 ...
		 */
			...
			this.byId("sel27").setValueState(sap.ui.core.ValueState.None);
		 ...
		 */
			this.byId("sel26a").setSelectedKey("");
			this.byId("sel26b").setSelectedKey("");
			this.byId("sel27").setSelectedKey("");
			if (this.getView().getModel("modelGruppoSelected")) {
				this.getView().getModel("modelGruppoSelected").setData("");
			}
		},
	
		onShow13: function (oEvent) {
			/** CALLED UPON FROM VIEW
			...
			var m1 = this.byId("sel26a").getSelectedKey();
			var m2 = this.byId("sel26b").getSelectedKey();
			if (this.getView().getModel("modelGruppoSelected")) {
			...
			if (this.byId("sel26a").getSelectedKey() === "") {
				monthStart = "04";
			...
			if (this.byId("sel26b").getSelectedKey() === "") {
				monthEnd = "03";
			...
			if ((this.byId("sel26a").getSelectedKey() === "") && (this.byId("sel26b").getSelectedKey() === "")) {
				monthDay = "31";
				var year = this.selYear.split("-");
			...
			if (yearStart > yearEnd) {
				this.byId("sel26a").setValueStateText("Please Enter DAL MESE");
				this.byId("sel26b").setValueStateText("Please Enter AL MESE");
			...
			oBaseModel.read('/V_REPORT2V2Set', {
				filters: aFilter,
				...
				success: function (oDataIn, oResponse) {
					...
					if (oDataIn.results.length > 0) {
						...
						var oModelReport2 = new sap.ui.model.json.JSONModel(oDataIn.results);
						that.getView().setModel(oModelReport2, "oModelReport2");
					} else {
						MessageToast.show('No Data Found on Selected Criteria');
						if (!oModelReport2) {
							that.getView().getModel("oModelReport2").setData(null);
						}
					}
				},
				error: function (oError) {
		...
		/***** CLEARS THE VALUES OF DAL MESE, AL MESE AND LOADED DATA *****/
			var that = this;
			that.byId("sel26a").setSelectedKey("");
			that.byId("sel26b").setSelectedKey("");
			that.byId("sel27").setSelectedKey("");
			that.getView().getModel("oModelReport2").setData(null);
		},
		 ...
		 */
			...
			tablesToExcel(obj, ['tab1'], columnTemplate, 'Report2.xls', 'Excel');
		},
	});
});