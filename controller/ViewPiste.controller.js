sap.ui.define(["sap/ui/core/mvc/Controller",
	...
	"use strict";
	var oResource;
	return t.extend("stipAdmin.stipAdmin.controller.ViewPiste", {
		onInit: function () {

			this.getOwnerComponent().getRouter().getRoute("ViewPiste").attachPatternMatched(this._onObjectMatched, this);
			...
			if (data1) {
				...
				for (var i = 0; i < data1.results.length; i++) {
					...
					if (i === 0) {
						...
						oEntry.ConsuntivoSimulato = data1.CONSUNTIVO_GRADINO;
						...
						oEntry.ConsuntivoSimulato = data1.results[i].CONSUNTIVO_GRADINO;
						oEntry.ObiettivoSimulato = data1.results[i].OBIETTIVO_GRADINO;
						oEntry.ObiettivoEnable = true;
						oEntry.ConsuntivoEnable = true;
						oEntry.ConsuntivoSimulatoEnable = true;
						oEntry.ObiettivoSimulatoEnable = true;
		...
		handleDescGatePress2: function (oEvent) {
			if (this.getView().byId("Mul2").getSelectedItem()) {
				...
				sap.ui.getCore().getModel("BasicAppModel").setProperty("/back", "ViewPiste");
		...
		calculate: function (data1, sServiceUrl, count) {
			...
			return oObject;
			// getting performance and mbo

			/*		var that = this;
					var oFilters = [];
					var year = "";
					//	var ID_CURVA = parseInt(sap.ui.getCore().getModel("PercMboModel").getProperty("/ID_CURVA"));
					if (data1.ID_CURVA != undefined && data1.ID_CURVA != "") {
						var filter1 = new sap.ui.model.Filter("ID_CURVA", sap.ui.model.FilterOperator.EQ, data1.ID_CURVA);
						oFilters.push(filter1);
					}
					...
					var mParameters = {
						...
						success: function (oDataIn, oResponse) {
							...
							for (var i = 0; i < oResponse.data.results.length; i++) {
								...
								oEntry.VALORE_CURVA = oResponse.data.results[i].VALORE_CURVA;
								arrayOentry.push(oEntry);
								oEntry = {};
							}
							that.calculate_PistewithoutSottopiste(arrayOentry, data1);
						},
						error: function (oError) {
							MessageBox.error("Error in getting Gruppo Pista. Please contact administrator.");
						}
