sap.ui.define(["sap/ui/core/mvc/Controller", "stipAdmin/stipAdmin/util/Formatter",
	...
	var oMainModel = new sap.ui.model.json.JSONModel();
	var idCurve = "";
	return Controller.extend("stipAdmin.stipAdmin.controller.modCurvaLineare_Consuntivo", {
		/******************** onInit method is called upon initialization of the View. The controller can perform its internal setup in this hook ********************************************/
		onInit: function () {
			this.getOwnerComponent().getRouter().getRoute("modCurvaLineare_Consuntivo").attachPatternMatched(this._onObjectMatched, this);
		},
		...
		validation: function () {
			...
			if (desc === "" || desc === undefined) {
				...
				for (var i = 0; i < curveLength; ++i) {
					...
					curveData[i].S_GRADINO = parseFloat(curveData[i].S_GRADINO)
					if (isNaN(curveData[i].PERC_MBO) || isNaN(curveData[i].S_GRADINO)) {	//Validation for performance and payout to be numbers only
		...
		/********************sendCurveDMSfile function is used to create the file type and sendFileDMS is called********************************************/
		sendCurveDMSfile: function () {
			...
			var filenameTXT = "CURVA--LineareConsuntivo--" + idCurve + ".txt";
