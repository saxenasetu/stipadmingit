sap.ui.define([
	...
	"use strict";
	var i;
	return e.extend("stipAdmin.stipAdmin.controller.KPIconConsuntivoMancante", {
		...
		onInit: function () {
			...
			this.oRouter.getRoute("KPIconConsuntivoMancante").attachPatternMatched(this._onObjectMatched, this);
		},

		onFlagChange: function () {
			/***** THIS FUNCTION IS CALLED BY THE COMBOBOX IN VIEW AND THE VALUE STATE IS GIVEN NONE *****/
			this.byId("sel30a").setValueState(sap.ui.core.ValueState.None);
			this.byId("sel30b").setValueState(sap.ui.core.ValueState.None);
		},
		...
		handleBack: function (oEvent) {
			...
			if (this.getView().getModel("oModelReport4")) {
				this.getView().getModel("oModelReport4").setData(null);
			}
			this.busyDialog.close();
		},
		...
		onValueHelpRequest1: function (oEvent) {
			...
			this.byId("sel31").setValueState(sap.ui.core.ValueState.None);
		...
		onResetta15: function (oEvent) {
			/***	Provides valueHelp for the GruppoKP input field based on the periodoId */
			this.byId("sel30a").setSelectedKey("");
			this.byId("sel30b").setSelectedKey("");
			this.byId("sel31").setSelectedKey("");
			if (this.getView().getModel("modelGruppoSelected")) {
				this.getView().getModel("modelGruppoSelected").setData("");
			}
		},

		onShow14: function (oEvent) {
			 ...
			 */
			var m1 = this.byId("sel30a").getSelectedKey();
			var m2 = this.byId("sel30b").getSelectedKey();
			if (this.getView().getModel("modelGruppoSelected")) {
			...
			var monthEnd;
			var monthDay;
			if (this.byId("sel30a").getSelectedKey() === "") {
				monthStart = "04";
			...
			if (this.byId("sel30b").getSelectedKey() === "") {
				monthEnd = "03";
			...
			if ((this.byId("sel30a").getSelectedKey() === "") && (this.byId("sel30b").getSelectedKey() === "")) {
				monthDay = "31";
				var year = this.selYear.split("-");
			...
			if (yearStart === "") {
				yearStart.setSelectedKey("Aprile");
			}
			if (yearStart > yearEnd) {
				this.byId("sel30a").setValueStateText("Please Enter DAL MESE");
				this.byId("sel30b").setValueStateText("Please Enter AL MESE");
			...
			oBaseModel.read("/V_REPORT4V2SetNew", {
				filters: aFilter,
				...
				success: function (oDataIn, oResponse) {
					...
					if (oDataIn.results.length > 0) {
						...
						var oModelReport4 = new sap.ui.model.json.JSONModel(oDataIn.results);
						that.getView().setModel(oModelReport4, "oModelReport4");

					} else {
						MessageToast.show("No Data Found on Selected Criteria");
						if (!oModelReport4) {
							that.getView().getModel("oModelReport4").setData(null);
						}
					}
		...
		clearValues: function () {
			...
			var that = this;
			that.byId("sel30a").setSelectedKey("");
			that.byId("sel30b").setSelectedKey("");
			that.byId("sel31").setSelectedKey("");
			that.getView().getModel("oModelReport4").setData(null);
		...
		onPressExport: function (oEvent) {
			...
			var oModel = this.getView().getModel("oModelReport4");
			var columnTemplate = [
				[{
					column: "ID_PISTE",
					label: "Id KPI"
				...
				**/
				...
				},
				{
					...
					column: "Stato persona",
					label: "Stato persona"
				} ]
			];
			var obj = {};
			obj.results = oModel.getData();
			tablesToExcel(obj, ["tab1"], columnTemplate, "Report4.xls", "Excel");
		},
	});
});