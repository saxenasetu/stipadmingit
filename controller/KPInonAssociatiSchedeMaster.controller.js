sap.ui.define([
	...
	"use strict";
	var i;
	return e.extend("stipAdmin.stipAdmin.controller.KPInonAssociatiSchedeMaster", {
		...
		/**** ONINIT FUNCTION IS CALLED UPON INITIALIZATION OF THE VIEW AND CONTROLLER PERFORM THE SETUP ****/
			...
			this.oRouter.getRoute("KPInonAssociatiSchedeMaster").attachPatternMatched(this._onObjectMatched, this);
		},
		
		onFlagChange: function () {
		/***** THIS FUNCTION IS CALLED BY THE COMBOBOX IN VIEW AND THE VALUE STATE IS GIVEN NONE *****/
			this.byId("sel21a").setValueState(sap.ui.core.ValueState.None);
			this.byId("sel21b").setValueState(sap.ui.core.ValueState.None);
		},
		 ...
		 */
			...
			this.byId("sel22").setValueState(sap.ui.core.ValueState.None);
		...
		onShow12: function (oEvent) {
			...
			var m1 = this.byId("sel21a").getSelectedKey();
			var m2 = this.byId("sel21b").getSelectedKey();
			if (this.getView().getModel("modelGruppoSelected")) {
			...
			if (this.byId("sel21a").getSelectedKey() === "") {
				monthStart = "04";
			...
			if (this.byId("sel21b").getSelectedKey() === "") {
				monthEnd = "03";
			...
			if ((this.byId("sel21a").getSelectedKey() === "") && (this.byId("sel21b").getSelectedKey() === "")) {
				monthDay = "31";
				var year = this.selYear.split("-");
			...
			if (yearStart > yearEnd) {
				this.byId("sel21a").setValueStateText("Please Enter DAL MESE");
				this.byId("sel21b").setValueStateText("Please Enter AL MESE");
			...
			oBaseModel.read('/V_REPORT1V2SetNew', {
				filters: aFilter,
				...
				success: function (oDataIn, oResponse) {
					...
					if (oDataIn.results.length > 0) {
						...
						var oModelReport1 = new sap.ui.model.json.JSONModel(oDataIn.results);
						that.getView().setModel(oModelReport1, "oModelReport1");
					} else {
						MessageToast.show('No Data Found on Selected Criteria');
						if (!oModelReport1) {
							that.getView().getModel("oModelReport1").setData(null);
						}
					}
		...
		/***** CLEARS THE VALUES OF DAL MESE, AL MESE AND LOADED DATA *****/
			var that = this;
			that.byId("sel21a").setSelectedKey("");
			that.byId("sel21b").setSelectedKey("");
			that.byId("sel22").setSelectedKey("");
			that.getView().getModel("oModelReport1").setData(null);
		...
		onPressExport: function (oEvent) {
			...
			var oModel = this.getView().getModel('oModelReport1');
			var columnTemplate = [
				[{
					column: 'ID_PISTE',
			...
			tablesToExcel(obj, ['tab1'], columnTemplate, 'Report1.xls', 'Excel');
		},
	});
});