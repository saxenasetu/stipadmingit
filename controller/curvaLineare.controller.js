...
], function (t, MessageBox, RepoService) {
	"use strict";
	var minPercMbo;
	var maxPercMbo;
	var curveIdModel = new sap.ui.model.json.JSONModel();
	return t.extend("stipAdmin.stipAdmin.controller.curvaLineare", {
		/******************** onInit method is called upon initialization of the View. The controller can perform its internal setup in this hook ********************************************/
		onInit: function () {
			this.getOwnerComponent().getRouter().getRoute("curvaLineare").attachPatternMatched(this._onObjectMatched, this);
		...
		validation: function () {
			...
			if (desc === "" || desc === undefined) {
				...
				var pattern = RegExp('[0-9]');		//This pattern allows numbers only
		...
		createLinearCurve: function (curveResultJson, desc, periodiId, selectedCurve) {
			...
			$.ajax({
				...
				success: function (data, textStatus1) {
					...
					that.getView().setModel(curveIdModel, "displayCurveLinearModel");
		...
		/******************** displayLinearCurve function is used to plot  and display the Linear curve based 
		 on dimension(X-axis) as performance and measure(Y-axis) as payout********************************************/
		/*displayLinearCurve: function (desc, curveId) {
			//	debugger
			console.log(curveId);
			...
			console.log(data);
			data.sort(function (a, b) {	//Sorting performance values in X-axis 
				if (Number(a.Payout) < Number(b.Payout)) {
					return -1;
				} else
					return 1;
			});
		...
		displayLinearCurve: function (curveDesc) {
			//	debugger
			var that = this;
			this.getView().byId("idcolumn").setVisible(true);
			//      1.Get the id of the VizFrame
		...
		/********************sendCurveDMSfile function is used to create the file type and sendDMS function is called********************************************/
		sendCurveDMSfile: function () {
			...
			var filenameTXT = "CURVA--LINEARE--" + curveId + ".txt";
